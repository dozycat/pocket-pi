// node:crypto over pure JS (bundler alias). Guests use createHash for
// content-addressed cache keys (e.g. cat's memo store), so what matters is
// a stable, collision-resistant digest — this is a real SHA-256.
...
export const createHash = (algorithm) => {
  if (String(algorithm).toLowerCase() !== "sha256") {
    throw new Error(`pocket-pi node:crypto shim supports sha256 only (got ${algorithm})`)
  }
  const parts = []
  return {
    update(data) {
    ...
    digest(encoding) {
      ...
      const hex = sha256(all)
      if (encoding !== "hex") throw new Error("pocket-pi node:crypto shim: hex digests only")
      return hex
    }
  }
}
